...
$here = static function (string ...$segments): string {
  return implode(DIRECTORY_SEPARATOR, array_merge([realpath(__DIR__)], $segments));
};
$manifest = json_decode(file_get_contents($here('composer.json')));

if (!property_exists($manifest, 'name')) {
    throw new LogicException('composer.json is missing the project `name` field');
}

if (!property_exists($manifest, 'authors') || count($manifest->authors) <= 0) {
    throw new LogicException('composer.json is missing the project `authors` field');
}

if (!property_exists($manifest->authors[0], 'name') || !property_exists($manifest->authors[0], 'email')) {
    throw new LogicException('composer.json `authours` appear to be missing the `name` and/or `email` field');
}

if (!property_exists($manifest, 'license')) {
    throw new LogicException('composer.json is missing the `license` field');
}
...
return $config
    ...
    ->setRules([
      ...
      */
      'header_comment' => [
        'header' => sprintf(
          "This file is part of the %s PHP library.\n\n@copyright %d %s <%s>\n@license %s\n\nFor the full copyright and license information, please view the LICENSE\nfile that was distributed with this source code.",
          $manifest->name,
          date('Y'),
